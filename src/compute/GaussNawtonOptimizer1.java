...
public class GaussNawtonOptimizer1 {
	...
	public static double[] getInitialValue(ArrayList<Vector2D> observed) {
		...
		System.out.println(svd.getV().getColumn(3)[3]);
		System.out.println(svd.getU().getColumn(3)[3]);
		...
		double z[] = { -b1/(2*a), -b2/(2*a)};
		double r = Math.sqrt((b1*b1 + b2*b2)/(4*a*a) - (c/a));
	...
	public static double[] getCircle(ArrayList<Vector2D> observed) {
		...
		MultivariateJacobianFunction distancesToCurrentCenter = new MultivariateJacobianFunction() {
			public Pair<RealVector, RealMatrix> value(final RealVector point) {
				...
				RealVector value = new ArrayRealVector(observed.size());
				RealMatrix jacobian = new Array2DRowRealMatrix(observed.size(), 3);
